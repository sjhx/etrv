...
void HRF_reg_Rn(uint8_t *retBuf, uint8_t addr, uint8_t size){
	...
	spi_transfern((char*)retBuf, size + 1);
	retBuf[0] = tmp;
	return;
}
void HRF_reg_Wn(uint8_t *retBuf, uint8_t addr, uint8_t size){
	...
	spi_writenb((char*)retBuf, size + 1);
	retBuf[0] = tmp;
	return;
}
...
uint8_t* HRF_make_FSK_msg(uint32_t sensorId, uint8_t paramNum, ...){
	...
	msgData[2] = 0x04;
	msgData[3] = 0x01;						// Product id: 0x02 R1 (monitor + control); 0x01 C1 (monitor only)
	msgData[4] = 1;//rand();
	msgData[5] = 0;//rand();
...
void cryptMsg(uint8_t *buf, uint8_t size){								// encrypt and decrypt message
	...
	seed(g_seed_pid, (uint16_t)(buf[SEED_PID_POSITION]<<8)|buf[SEED_PID_POSITION + 1]);
